...
RDesk::build_app(
  ...
  # runtime_dir = NULL (default)     # copies YOUR R version automatically
  # runtime_dir = "download"         # legacy: downloads R from CRAN (not recommended)
)
# Output: dist/MyApp-1.0.0-setup.exe
# Runtime: your exact R version is bundled - no version mismatch with renv
```
...
Key v1.0.5 change:
  build_app() now copies the developer's installed R as the runtime
  by default (runtime_dir = NULL). This eliminates the version mismatch
  between renv-locked packages and the previously downloaded R 4.4.2.
  Use runtime_dir = "download" to restore the old download behaviour.
```
