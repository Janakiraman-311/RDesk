...
Do not develop RDesk from a folder synced by OneDrive or similar.
The g++ compiler reads stale cached source files from synced folders,
producing ghost bugs where code changes have no effect.
Always work from a local non-synced directory such as C:/Projects/RDesk.
The build system copies source to a temp directory to mitigate this,
but working locally is still strongly recommended.

## Building the launcher

The C++ launcher is compiled automatically by build_app().
To compile manually for development:
...
`vignettes/ipc-contract.Rmd` for the full specification.
Never break this contract without bumping the version in zzz.R.
